...
def test_mcp_interceptor():
    # Prepare requests
    req1 = json.dumps({"jsonrpc": "2.0", "id": 1, "method": "initialize", "params": {"protocolVersion": "2024-11-05"}}) + "\n"
    req2 = json.dumps({"jsonrpc": "2.0", "id": 2, "method": "tools/call", "params": {"name": "fetch_data", "arguments": {"id": 123}}}) + "\n"
    ...
    runs = list(db["runs"].rows_where("agent_name = ?", ["mcp_server"], order_by="start_time DESC"))
    assert len(runs) >= 1
    run_id = runs[0]["id"]
